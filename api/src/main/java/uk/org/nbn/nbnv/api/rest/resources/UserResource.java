...
public class UserResource extends AbstractResource {
	...
	public Response createTokenCookie(
		...
		return doCreateTokenCookie(username, password, remember);
	}
	 ...
	 * @throws ExpiredTokenException Token has expired
	 */
	private Response doCreateTokenCookie(String username, String password, Boolean remember)
		...
		try {
			...
			logger.warn("Caught a possible downed core server login, check core database is up; " + ex.getLocalizedMessage());
		}
	...
	/**
	 * Search for users in the core database, excluding those who are members of
	 ...
	 * @param key The activation key from the email sent to the new users email
	 * account
	 ...
	 *
	 * @param user The current user must be logged in (Injected Token no need to
	 * pass)
