...
@Provider
@Component
public class TokenOrganisationAccessRequestAdminUserProvider implements InjectableProvider<TokenOrganisationAccessRequestAdminUser, Type> {   
    @Autowired private DatasetAdministratorMapper datasetAdministratorMapper;
    @Autowired private OperationalOrganisationAccessRequestMapper oOrganisationAccessRequestMapper;
    ...
    @Override public Injectable<User> getInjectable(ComponentContext cc, TokenOrganisationAccessRequestAdminUser a, Type c) {
    ...
    private class UserInjector implements Injectable<User> {
        private final TokenOrganisationAccessRequestAdminUser userAnnot;
        
        private UserInjector(TokenOrganisationAccessRequestAdminUser userAnnot) {
        ...
        @Override public User getValue() {
            ...
            String datasetKey = oOrganisationAccessRequestMapper.getRequest(Integer.parseInt(request.getPathParameters().getFirst(userAnnot.path()))).getDatasetKey();
