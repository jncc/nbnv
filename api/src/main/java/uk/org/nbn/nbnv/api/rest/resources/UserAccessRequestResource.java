...
public class UserAccessRequestResource extends RequestResource {
    ...
    @Autowired
    OperationalTaxonObservationFilterMapper oTaxonObservationFilterMapper;
    @Autowired
    ...
    @Autowired
    OperationalUserAccessRequestAuditHistoryMapper oUserAccessRequestAuditHistoryMapper;
    @Autowired
    DatasetAdministratorMapper datasetAdministratorMapper;
    @Autowired
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public UserAccessRequest getRequest(@TokenAccessRequestAdminUser(path = "id") User user, @PathParam("id") int filterID) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Response updateRequest(@TokenAccessRequestAdminUser(path = "id") User user, @PathParam("id") int filterID, @FormParam("action") String action, @FormParam("reason") String reason, @FormParam("expires") @DefaultValue("") String expires) throws ParseException, IOException, TemplateException {
    ...
    /**
     * SysAdmin function to get all expired access requests, used to find
     * expired requests
     *
    ...
    /**
     * Returns an audit history listing all organisational access changes made
     * to a dataset
     *
     ...
     * List<OrganisationAccessRequestAuditHistory>
     * @response.representation.200.mediaType application/json
     */
    @GET
    ...
    private Response acceptRequest(User user, int filterID, String reason, String expires, boolean proactive) throws ParseException, IOException, TemplateException {
        ...
        if (expires.isEmpty()) {
            if (!accessRequest.getTime().isAll()) {
                editExpiryDate(user, filterID, uar, accessRequest, null);
            }
            ...
            if (accessRequest.getTime().getDate() != null) {
                ...
                if (!(expiresCal.get(Calendar.YEAR) == requestCal.get(Calendar.YEAR)
                    ...
                    editExpiryDate(user, filterID, uar, accessRequest, expiresDate);
                }
            } else {
    ...
    private void editExpiryDate(User user, int filterID, UserAccessRequest uar, AccessRequestJSON accessRequestJSON, java.util.Date expiresDate) throws IOException {       
