...
public class TaxonDatasetMetadataArchiveService {
    ...
    public TaxonDataset readWordDocument(File input, Organisation organisation) throws IOException {
        try (ZipFile zipFile = new ZipFile(input)) {
            ...
            //Set the contact email to that as supplied in the additons
            organisation.setContactEmail(additions.getAdminEmail());
            taxonDataset.setOrganisation(organisation);
            taxonDataset.setOrganisationID(organisation.getId());
            taxonDataset.setOrganisationName(organisation.getName());
