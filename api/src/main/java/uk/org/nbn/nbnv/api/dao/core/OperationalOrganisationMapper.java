...
public interface OperationalOrganisationMapper {
    @Select("SELECT CASE WHEN logo IS NULL THEN 0 ELSE 1 END as hasLogo, CASE WHEN logoSmall IS NULL THEN 0 ELSE 1 END as hasSmallLogo, id, name, abbreviation, summary, address, postcode, contactName, contactEmail, website, allowPublicRegistration FROM Organisation WHERE id = #{id}")
    ...
    @Update("UPDATE Organisation SET name = #{name}, abbreviation = #{abbreviation}, "
            ...
            + "contactEmail = #{contactEmail}, allowPublicRegistration = #{allowPublicRegistration} "
            + "WHERE id = #{id}")
    int updateOrganisationDetails(
            ...
            @Param("allowPublicRegistration") int allowPublicRegistration);
