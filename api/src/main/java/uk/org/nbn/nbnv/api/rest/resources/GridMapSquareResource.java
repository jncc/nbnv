...
public class GridMapSquareResource extends AbstractResource {
	...
	public StreamingOutput getGridMapSquares(
		...
		return new StreamingOutput() {
			...
			public void write(OutputStream out) throws IOException, WebApplicationException {
				...
				try {
					addReadMe(zip, user, ptvk, resolution, bands, viceCountyIdentifier, verifications);
	 ...
	 * @throws IOException
	 */
	private void addReadMe(ZipOutputStream zip, User user, String ptvk, String resolution, List<String> bands, String viceCountyIdentifier, List<String> verifications) throws IOException {
	...
	/**
	 * @param bands list of Strings with year range and colours, eg:
	 * 2000-2012,ff0000,000000
	 * @param list of verification statuses either with colours (eg
	 * 1,ff0000,00ff00), or without (eg 1)
	 */
	...
	private void addDatasetMetadata(ZipOutputStream zip, User user, String ptvk, String resolution, List<String> bands, List<String> datasetKeys, String viceCountyIdentifier, List<String> verifications) throws IOException {
		List<Integer> verificationKeys = getVerificationKeys(verifications);
		boolean isGroupByDate = isGroupByDate(verifications);
		List<TaxonDataset> taxonDatasets = gridMapSquareMapper.getGridMapDatasets(user, ptvk, resolution, getStartYear(bands), getEndYear(bands), datasetKeys, viceCountyIdentifier, verificationKeys, isGroupByDate);
