...
-->
            ...
            <body>
                <h1>
                    <xsl:choose>
                        ...
                        <xsl:otherwise>NBN V API</xsl:otherwise>
                    </xsl:choose>
                </h1>
    ...
    <xsl:template match="wadl:resource" mode="toc">
		...
		<xsl:variable name="methods"><xsl:apply-templates select="wadl:method" mode="toc"/></xsl:variable>
        <li><span class="toc"><a href="#{$id}"><xsl:value-of select="$name"/></a><span style="float:right;"><xsl:value-of select="$methods"/></span></span>
	...
	<xsl:template match="wadl:method" mode="toc">
		[<xsl:value-of select="@name"/>]
    </xsl:template>
