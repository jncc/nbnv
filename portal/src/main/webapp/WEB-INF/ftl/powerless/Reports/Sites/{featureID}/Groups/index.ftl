...
<@template.master title="NBN Site Report" 
    ...
    <h1>${title}</h1>
    <form id="nbn-site-report-form" api-server="${api}" featureID="${featureID}" gridSquare="${is10kmReport?string}">
        <@report_utils.site_report_filters requestParameters=RequestParameters location=site.label  isSpatialRelationshipNeeded=true isDownloadButtonNeeded=true isRequestBetterAccessLinkNeeded=true isVerificationNeeded=true/>
