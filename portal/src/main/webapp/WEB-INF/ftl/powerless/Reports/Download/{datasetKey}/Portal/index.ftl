...
<@template.master title="Portal Download Report For ${dataset.title}"
    ...
    <div id="nbn-download-stats-div" class="tabbed nbn-organisation-tabbed">
        <h3>Information</h3>
        <p>The downloads shown below are all cases where a user has viewed record data through either the portal or directly through the API, rather than through the the provided download wizard. This can only be done from the Taxon Observations Pages where are user selects an individual species or via the interactive mapper. When users use the API they can also view record data, so these views are also recorded here.</p>
        <p>These downloads are all attached to the filters that were used to view these records and how many records where viewed, we cannot however record the use of the records through this system. The level of access given to the records is determined by the level of access granted to the user in question.</p>
    </div>
