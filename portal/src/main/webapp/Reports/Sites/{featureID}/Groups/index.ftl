...
<@template.master title="Site report for..." javascripts=["/js/site-report-form-validation.js","/js/jquery.dataset-selector-utils.js"]>
    ...
    <form action="" method="post" id="nbn-site-report-form">
        <@report_utils.site_report_filters requestParameters=RequestParameters location=site.label isSiteBoundaryReport=true/>
        <div class="nbn-report-data-container">
            <table class="nbn-coloured-table">
                ...
                <#list taxonOutputGroupsWithQueryStats as taxonOutputGroupWithQueryStats>
                    <tr><td class="nbn-td-left nbn-td-right"><a href="/Reports/Sites/${featureID}/Groups/${taxonOutputGroupWithQueryStats.taxonGroupKey}/Species${report_utils.getQueryParameterText(RequestParameters)}">${taxonOutputGroupWithQueryStats.taxonOutputGroup.taxonGroupName}</a>(${taxonOutputGroupWithQueryStats.querySpecificSpeciesCount})</td></tr>
                </#list>
            </table>
        </div>
