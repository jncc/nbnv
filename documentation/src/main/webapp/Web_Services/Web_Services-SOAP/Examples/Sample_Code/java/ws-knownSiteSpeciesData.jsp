...
<p>This example demonstrates using the OneSiteDataRequest web service to fetch records and a
    distribution map for a single species in a known site. It also uses the KnownSiteNameRequest
...
try {
    ...
    if (siteKey != null && siteKey.length() > 0) {
        ...
        // Create the geographical filter and add the KnownSite object to it.
        ...
        for(int i = 0; i < sites.size(); i++) {
            siteMap.put(((Site)sites.get(i)).getSiteKey(),sites.get(i));
        }
        ...
        for(int i = 0; i < species.size(); i++) {
            speciesMap.put(((Species)(species.get(i))).getTaxonVersionKey(),species.get(i));
        } %>
        <div id="mapWrapper">
        <div id="mapImage">
